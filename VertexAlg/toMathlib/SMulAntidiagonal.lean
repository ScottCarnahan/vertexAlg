...
@[to_additive]
theorem smulAntidiagonal'_mono_left {s u : Set G} {t : Set P} (a : P) (h : u ⊆ s)
...
@[to_additive]
theorem smulAntidiagonal'_mono_right {s : Set G}
...
@[to_additive]
theorem support_smulAntidiagonal'_subset_smul {s : Set G}
...
theorem isPWO_support_smulAntidiagonal' :
  ...
  (hs.smul ht).mono
    (support_smulAntidiagonal'_subset_smul (fun a ↦ (Set.SMulAntidiagonal.finite_of_isPWO hs ht a)))

end

@[to_additive]
theorem smulAntidiagonal'_min_smul_min [LinearOrder G] [LinearOrder P] [SMul G P]
